...
This markdown will use analysis in Part 2 as an example, 
however the same techniques were applied for Part 1 with 
names swapped to match experimental groups used (Control DMFu VEGF VEGFDMFu).
...
### Run DESeq2
DESeq2 was utilised to perform repeated wald tests to identify significantly differentially expressed genes. 
Several output objects were generated for future use: 
...
For future analysis I have used a naming convention here which matches the comparison type set for the specific figure.

```{r}

#name <- "important_groups"

#saveRDS(dds, file= paste0(in_path,name,"dds.RDS"))  #unhash to save RDS

#write.csv(normcounts, file = paste0(in_path,name,"ggs_nocutoff_normalised_001.csv"))


```

## Check for expression of Endothelial Marker Genes
...
gene_set <- "AMD_voigt"
markers <- "choroid_endo"


# Filter based on lists ---------------------------------------------------

  #Read in data
data <- normcounts #Read in normalised counts
  #annotate with current gene symbols
data <- data %>%
  mutate("symbol" = mapIds(org.Hs.eg.db,
                           keys = rownames(data), 
                           keytype = "ENSEMBL", 
                           column = "SYMBOL"))%>%  #read in gene symbols
  relocate(symbol) %>%  #relocate symbols to 2nd column
  mutate(X = NULL) #remove ensembl IDs
  #use if pulling in the fibro data
#colnames(data)[colnames(data) == "IL.6_1"] <- "symbol"
  
#data <- data %>%
  #dplyr::relocate(symbol)%>%
  #mutate(ID = NULL)
  
filt <- read.csv(paste0("reference_data/full_lists.csv"), header = TRUE) #Read in filters with each filter as a seperate column


Filters <- lapply(filt, function(x) data.frame(x)) #split each column into its own dataframe

#create a list of filter names to loop through
filters <- colnames(filt)
i <- "EC.Marker"
lists
...
for (i in filters) {
  ...
  v <- gsub(" ","", gsub("[ \t]", "",(filt[[i]]))) # Trim whitespace
  v <- v[v != ""]# Remove empty strings
  df <- data[data$symbol %in% v, ]
  
  missing_symbols <- setdiff(v, data$symbol) #check for values in list that are missing

  if(length(missing_symbols)>0){
    print(paste0("Expression data for: ", missing_symbols," is missing from the normalised dataset, please confirm symbols if symbols are correct or if values are 0"))
      missing_df <- tibble(symbol = missing_symbols)
      missing_df[, colnames(data)[-1]] <- 0
  } else{
      missing_df <- NULL
    }
...
df_long <- df %>%
  pivot_longer(cols = -symbol, names_to = "Treatment", values_to = "Expression") %>%
  mutate(Treatment = gsub("_\\d+", "", Treatment)) %>%
  mutate(Treatment = gsub("All", "Cocktail", Treatment)) %>%
  group_by(symbol, Treatment) %>% #change to long form 
  mutate(RelativeExpression = (Expression / sum(Expression))*100)  #Change to relative expression calculation

df_long$Treatment <- factor(df_long$Treatment, #reorder to match the rest of the paper
                            levels = c("Control","VEGF","IL.6","TGFb1","TGFb2", "TNFa", "Thrombin", "Cocktail"))

boxplot <- ggplot(df_long, aes(x = Treatment, y = Expression, fill = Treatment)) +
                  geom_boxplot(outliers = FALSE) +
                  facet_wrap(~ symbol, scales = "free", nrow = 1, ncol = 4) +
                  theme(axis.text.x = element_text(angle = 90, hjust = 1, vjust= 0.5, size = 18),
                        axis.text.y = element_text(size = 18),
                        strip.text = element_text(size = 26),
                        legend.title = element_text(size = 26),
                        legend.text = element_text(size = 22),
                        legend.box.background = element_rect(fill = NULL, colour = "grey", linewidth = 2)) +
                  labs(title = paste0(i, " associated gene expression" ),
                       x = NULL, 
                       y = "Expression") +
                  scale_fill_manual(values = c("Control" = "#666666",
                                               "VEGF" = "#7570B3",
                                               "IL.6" = "#E6AB02",
                                               "TGFb1" = "#66A61E",
                                               "TGFb2" = "#1B9E77", 
...
boxplot
length(v)
ggsave(paste0(out_path,i,"_relexpr_plot2.svg"), plot = boxplot, width = 35, height = 7.5*(length(v)/4), units = )
  }
...
#mycols <- c('#9d9da1','#89489b','#3eb548','#4097AB') #colours for DMFu analysis

mycols <- c("#666666","#A6761D","#E6AB02", "#66A61E", "#1B9E77","#E7298A", "#D95F02", "#7570B3") #multi-cyto colours

PCA <- plotPCA(vsdata, intgroup = "condition", ntop = 1000)+
  theme_bw()+ #base theme
  coord_equal(4)+ #set a plot size
  stat_ellipse(aes(group = condition), type = "norm", level = 0.95)+
...
Further style changes were made in an external vector editor to increase legibility for **Figure 1A**, these include: 
  - stylistic adjustment of ellipses  to bound points as a filled object 
...
#treatments <- c("VEGF","VEGFDMFu","DMFu") #for DMFu study
...
The following sections of code outline packages used and steps undertaken to 
identify biological themes and pathways enriched by each treatment. 
This includes:
...
Pathway enrichment was performed in *clusterProfiler*, 
both by direct referencing to downloaded GMT files as well as integrated referencing through *clusterProfiler*.
                                                  ...
                                                  gsub('mhc class i', 'MHC I', 
                                                    gsub('mhc class ii', 'MHC II', 
                                                      stringr::str_to_sentence(
                                                        gsub('_', ' ',  
                                                          gsub('BIOCARTA_|PID_|KEGG_MEDICUS_REFERENCE_|REACTOME_', '', 
res_df$Description
)))))))))))))))))))
...
Although useful in allowing a wide array of pathway databases to be investigated simultaneously, 
this method does not integrate effectively for visualisation, 
and was thus only run to assess if a treatment was eligible for ORA enrichment 
and assess the pathway categories reflected by major databases.
...
The following code will transform an enrichGO/KEGG/WP etc., object 
to a usable format for *GOChord()*
...
Although not presenteds, 
a clustered process map is a great way to visualise the K-means clustering performed
...
for (phenotype in Phenotypes){
  ...
  for (cytokine in cytokines) {
    ...
    write.csv(pheno_overlap, file = paste0(out_path,phenotype,"_overlapped_filtered_genes_1L2FC.csv"))
  }
```
...
df <- read.delim(paste0(in_path, "reference_data/", markers, ".csv"), header = TRUE, sep = ",")%>%
  select(!X) #remove ensembl IDs


# Filter based on lists ---------------------------------------------------

  #Read in data
data <- read.csv(paste0(in_path, "important_groups_ggs_normalised.csv"), header = TRUE) #Read in normalised counts
  #annotate with current gene symbols
data <- data %>%
  mutate("symbol" = mapIds(org.Hs.eg.db,
                           keys = data$X, 
...
filt <- read.csv(paste0(in_path, "full_lists.csv"), header = TRUE) #Read in filters with each filter as a seperate column
...
df_long <- df %>%
  ...
  mutate(Treatment = gsub("All", "Cocktail", Treatment)) %>%
  group_by(symbol, Treatment) %>% #change to long form 
  mutate(RelativeExpression = Expression / sum(Expression))  #Change to relative expression calculation

df_long$Treatment <- factor(df_long$Treatment, #reorder to match the rest of the paper
                            levels = c("Control", "TGFb2", "TNFa", "Thrombin", "Cocktail"))

boxplot <- ggplot(df_long, aes(x = Treatment, y = Expression, fill = Treatment)) +
                  geom_boxplot(outliers = FALSE) +
                  facet_wrap(~ symbol, scales = "free", nrow = 2, ncol = 5) +
                  theme(axis.text.x = element_text(angle = 90, hjust = 1, vjust= 0.5, size = 18),
                        axis.text.y = element_text(size = 18),
                        strip.text = element_text(size = 26),
                        legend.title = element_text(size = 26),
...
boxplot
length(v)
ggsave(paste0(out_path,i,"_plot.svg"), plot = boxplot, width = 40, height = 7.5*(length(v)/5))
  }
